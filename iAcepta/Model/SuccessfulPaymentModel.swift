//
//  SuccessfulPaymentModel.swift
//  iAcepta
//
//  Created by QUALITY on 7/9/19.
...
class SuccessfulPaymentModel: NSCoding {
...
//    }
    ...
    public func dictionaryRepresentation() -> [String: Any] {
        ...
        if let value = allDates { dictionary[ContentRequestModelKeys.allDates] = value }
        if let value = code { dictionary[ContentRequestModelKeys.code] = value }
        if let value = authorization { dictionary[ContentRequestModelKeys.authorization] = value }
        if let value = responseCode { dictionary[ContentRequestModelKeys.responseCode] = value }
        if let value = date { dictionary[ContentRequestModelKeys.date] = value }
        return dictionary
    }
