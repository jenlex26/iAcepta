...
class CircularSlider: UIControl {
  ...
  var typeImage : typeImage = .Propina{
        didSet{
            setNeedsDisplay()
        }
   }
...
//  }
  ...
  override func draw(_ rect: CGRect) {
    ...
    if handleType == .bigCircle{
        if(innerMarkingLabels != nil) {
            self.drawLine(ctx!)
        }
        self.drawHandle2(ctx!)
    }else{
  ...
  func drawHandle(_ ctx: CGContext, atPoint handleCenter: CGPoint) -> CGRect {
    ...
    handleColor = UIColor.blue//computedHandleColor
    handleColor!.set()
  ...
  func drawHandle2(_ ctx:CGContext){
    ctx.saveGState()
    handleColor = UIColor.blue
    handleColor!.set()
    let handleCenter = pointOnCircleAtAngleFromNorth(angleFromNorth)
    if(handleType == .bigCircle) {
        nombreImagen = ""
        
        if(typeImage == .Propina) {
            nombreImagen = "circulo_propina.png"
        }else if (typeImage == .Meses){
            nombreImagen = "circulo_mes"
        }
        let image = UIImage(named: nombreImagen)!
        let imageIconSlider:UIImage = self.rotateImage(image, angle: 0)
        contentScaleFactor = UIScreen.main.scale
        
        ctx.draw(imageIconSlider.cgImage!, in: CircularTrig.drawFilledCircleInContext(ctx, center: handleCenter, radius: 0.5 * handleWidth))
  ...
  func getRotationalTransform() -> CGAffineTransform {
    ...
    func rotateImage(_ img: UIImage, angle: CGFloat) -> UIImage {
        //Calculate the size of the rotated view's containing box for our drawing space
        let rotatedViewBox: UIView = UIView(frame: CGRect(x: 0, y: 0, width: img.size.width, height: img.size.height))
        let t: CGAffineTransform = CGAffineTransform(rotationAngle: angle * CGFloat.pi / 1) //180
        ...
        bitmap.rotate(by: (angle)) // * CGFloat.pi / 180
